...
ma.path.a <- "data/input/ma/star-ratings/Extracted Star Ratings/2021/2021 Star Ratings Data Table - Measure Stars (Oct 08 2020).csv"
star.data.a <- fread(
  ma.path.a,
  skip = 4,
  stringsAsFactors=FALSE,
  select=c(1:37),
  col.names=rating.vars.2021,
  na = c("", "NA", "*")
) %>%
  mutate(across(
...
ma.path.b <- "data/input/ma/star-ratings/Extracted Star Ratings/2021/2021 Star Ratings Data Table - Summary Rating (Oct 08 2020).csv"
star.data.b <- fread(
  ma.path.b,
  skip = 2,
  stringsAsFactors=FALSE, 
  select=c(1:11),
  col.names=c("contractid","org_type","contract_name","org_marketing","org_parent","snp","disaster_2018","disaster_2019","partc_score","partd_score","partcd_score"),
  na = c("", "NA", "*")
) %>%
  mutate(
